...
export default function RagCollectionsManager() {
  ...
  return <>
    <Card>
      ...
      <CardContent>{items.length === 0 ? <div className="rounded-xl border border-dashed p-8 text-center"><Layers3 className="h-7 w-7 mx-auto text-muted-foreground mb-2" /><p className="text-sm text-muted-foreground">Aucune collection déclarée.</p></div> : <div className="grid gap-3 md:grid-cols-2">{items.map((item) => <div key={item.id} className="rounded-xl border p-4 hover:border-primary/40 transition-colors"><div className="flex items-start justify-between gap-3"><div className="min-w-0"><div className="flex items-center gap-2 flex-wrap"><span className="font-semibold truncate">{item.name}</span><Badge variant={item.status === "active" ? "secondary" : "outline"}>{item.status || "active"}</Badge></div><div className="mt-2 rounded-md bg-muted/50 px-2 py-1 font-mono text-xs break-all">{item.collection_name}</div></div><div className="flex gap-1 shrink-0"><Button aria-label="Modifier" variant="ghost" size="sm" onClick={() => startEdit(item)}><Pencil className="h-3.5 w-3.5" /></Button><Button aria-label="Supprimer" variant="ghost" size="sm" className="text-destructive" onClick={() => remove(item)}><Trash2 className="h-3.5 w-3.5" /></Button></div></div><p className="text-xs text-muted-foreground mt-3 line-clamp-2">{item.description || "Sans description"}</p><div className="mt-3 flex flex-wrap gap-x-3 gap-y-1 text-[11px] text-muted-foreground"><span>Embedding : {item.embedding_model || "défaut"}</span><span>Métrique : {item.distance_metric || "cosine"}</span>{item.knowledge_base_id && <span>KB : {item.knowledge_base_id}</span>}</div></div>)}</div>}</CardContent>
    </Card>
